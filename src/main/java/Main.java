// Aviral Mishra
// TTT with 2D arrays
// Period 5
// 02/16/2024

/*
This project must play Tic-Tac-Toe.


For the first in-class demonstration, you must have the following:
1) a static constand 2-D Array the represnets the board and holds the STATE data for the game.
2) You must correctly resolve:
  horizontal, 
  diagonal, 
  vertical winner conditions
  And a draw
3) You must have 1 player.

For the future and more points above an A-...
0) You must correctly resolve all win and draw states
1) You should have a seond player
2) You should be able to play on a 4 x 4 board
3) You should correctly resolve turns
4) The player and the game should be in seperate classes from your static Main


For more in the future and more awesomer...
1) You can have a text file documenting player win-loss records and another stat of your choice
2) You can have a 3rd or 4th player
3) You can have a 3rd dimension to the game
4) You can make a new class for a game session as well as a single game
5) You can make a new class for a turn, or a piece, or a screen painter
*/

/**
* This is the main class where everything is put together
* Although the player and game information isn't held here, they are created here.
* The only method is to other than the main one is to interpret yes or no inputs
**/

import java.util.Scanner;

class Main {

  public static void main(String[] args) {
    Scanner console = new Scanner(System.in);

    // create player list
    int numberOfPlayers = 0;
    while (numberOfPlayers < 2){
      System.out.print("How many players are there total? ");
      numberOfPlayers = console.nextInt();
    }
    Player[] players = new Player[numberOfPlayers];

    // give players names
    System.out.println();
    for (int i = 0; i < players.length; i++){
      System.out.print("What is player " + (i + 1) + "'s name? ");
      players[i] = new Player(console.next());
    }

    // play the game
    boolean playMore = true;
    while (playMore){
      // get players to play
      System.out.println();
      Player playerX = null;
      while (playerX == null){
        System.out.print("Who wants to play now? ");
        String nameX = console.next();
        for (Player player : players){
          if (player.getName().equals(nameX))
            playerX = player;
        }
      }
      Player playerO = null;
      while (playerO == null){
        System.out.print("And who wants to play against them? ");
        String nameO = console.next();
        for (Player player : players){
          if (player.getName().equals(nameO))
            playerO = player;
        }
      }

      // start the game
      System.out.println();
      Game currentGame = new Game(console, playerX, playerO);
      System.out.println();
      currentGame.play();

      // check to play more
      System.out.println();
      System.out.print("Does anyone want to play? ");
      playMore = yesOrNo(console.next());
    }

    // retrieve stats
    for (Player player : players)
      System.out.println("\n" + player);
    
  }

  // interprets yes or no inputs
  public static boolean yesOrNo(String input){
    if (input.substring(0,1).equalsIgnoreCase("y"))
      return true;
    return false;
  }
}